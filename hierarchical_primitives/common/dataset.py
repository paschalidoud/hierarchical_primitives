...
class ImageInput(BaseDataset):
    ...
    def _get_item_inner(self, idx):
        return np.transpose(self._dataset_object[idx].random_image, (2, 0, 1))
...
class PointsOnMesh(BaseDataset):
    ...
    def __init__(self, dataset_object, transform=None):
        super(PointsOnMesh, self).__init__(dataset_object, transform)
...
class PointsAndLabels(BaseDataset):
    ...
    def __init__(self, dataset_object, transform=None):
        super(PointsAndLabels, self).__init__(dataset_object, transform)
...
def dataset_factory(name, dataset_object):
    ...
    image_input = ImageInput(
        dataset_object,
        transform=Compose([to_tensor, norm])
    )
